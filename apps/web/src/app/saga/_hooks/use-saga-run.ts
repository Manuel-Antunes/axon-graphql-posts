...
export function useSagaRun() {
    ...
    const push = useCallback((event: Omit<SagaEvent, "at">) => {
        setEvents((current) => [...current, { ...event, at: Date.now() - startedAt.current }]);
    }, []);

    const observe = useCallback(
        async (id: string) => {
            setStatus("waiting");
            while (!cancelled.current && Date.now() - startedAt.current < TIMEOUT_MS) {
    ...
    const run = useCallback(async () => {
        ...
        try {
            ...
            push({
                label: "SNS → SQS",
                detail: "posts.PostPreCreated saiu pelo outbox. O serviço de tagueamento decide agora.",
                tone: "pending",
            });
    ...
    const stop = useCallback(() => {
        ...
        setStatus((current) => (current === "waiting" || current === "creating" ? "idle" : current));
    }, []);
