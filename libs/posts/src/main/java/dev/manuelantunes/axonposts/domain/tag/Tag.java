...
public class Tag {
    ...
    /**
     * A identidade da tag padrão, <b>derivada do nome dela</b>.
     *
     * <h3>Por que derivar em vez de sortear</h3>
     * Porque ela precisa ser a MESMA em toda atribuição, para sempre, e sem estado para consultar. Se
     * cada post recebesse uma tag nova, o banco acumularia uma linha chamada {@value #DEFAULT_NAME} por
     * post — centenas, todas diferentes. A alternativa óbvia seria um catálogo ("qual é o id da tag
     * padrão?") e uma consulta antes de decidir; derivar do nome dá a mesma garantia sem catálogo
     * nenhum: a função é pura, e o resultado é idêntico em todo nó, em toda reinicialização e em todo
     * serviço que precise dela.
     * <p>
     * {@code nameUUIDFromBytes} é UUID versão 3 (MD5 de um nome). MD5 aqui não é escolha de criptografia
     * e não protege nada: é função de hash para gerar identidade estável a partir de um nome — o mesmo
     * papel de um UUIDv5, e é o que o JDK oferece de fábrica. O prefixo {@code tag:} existe para o
     * espaço de nomes não colidir com outro uso do mesmo nome; derivar ids de strings cruas é como dois
     * domínios diferentes acabam com o mesmo UUID por acidente.
     *
     * <h3>Por que aqui, e não em {@code TagId}</h3>
     * Porque "quem é a tag padrão" é uma resposta só, e ela tem duas metades — o nome e a identidade.
     * Separá-las obrigaria o {@code TagId} a importar a {@code Tag} para conhecer o nome, invertendo a
     * dependência: o value object de identidade passaria a depender da entidade. Mantidas juntas, o
     * {@code TagId} continua sendo um tipo de identidade e nada mais.
     * <p>
     * E é <b>do domínio</b>, não de quem atribui: dois serviços decidem a tag padrão (o de tagueamento,
     * em produção, e o dublê em processo, em teste) e os dois têm de chegar ao mesmo id. Deixar a regra
     * em um deles faria o outro copiá-la.
     */
