...
# Depois da migração para o Keycloak, toda mutation exige token. O TOKEN é preenchido por
# fetch_token() logo antes das chamadas; queries e subscriptions continuam abertas.
TOKEN=""
gql()  {
  if [ -n "$TOKEN" ]; then
...
# Token do autor semeado no realm. O grant `password` só existe porque o cliente do realm tem
# directAccessGrantsEnabled — é o que torna a POC roteirizável.
fetch_token() {
  TOKEN=$(curl -s -X POST "${KEYCLOAK_URL:-http://localhost:8081}/realms/axon-posts/protocol/openid-connect/token" \
    -d grant_type=password -d client_id=axon-posts-api \
    -d "username=${SMOKE_USER:-manuel@example.com}" -d "password=${SMOKE_PASSWORD:-segredo123}" \
    | python3 -c 'import json,sys; print(json.load(sys.stdin).get("access_token",""))')
  [ -n "$TOKEN" ] || { echo "FALHOU: Keycloak não devolveu token — o docker compose está de pé?"; exit 1; }
}
...
sse '{"query":"subscription { onPostCreated { id title author { name } version } }"}' > "$LOG/10-sse-onPostCreated.txt" 2>&1 &
SUB_PIDS="$!"
...
# a partir daqui as chamadas vão autenticadas: o autor sai do token, não do input
fetch_token
step "token do Keycloak obtido para ${SMOKE_USER:-manuel@example.com}"

gql '{"query":"mutation { createPost(input:{title:\"Axon + GraphQL over SSE\", content:\"primeiro post\"}) { id title author { name } createdAt version tags(first: 5) { edges { cursor node { id name } } pageInfo { hasNextPage } } } }"}' > "$LOG/20-createPost-A.txt"
A=$(post_id "$LOG/20-createPost-A.txt")
gql '{"query":"mutation { createPost(input:{title:\"Segundo post\", content:\"outro conteúdo\"}) { id title author { name } version tags(first: 5) { edges { cursor node { id name } } pageInfo { hasNextPage } } } }"}' > "$LOG/21-createPost-B.txt"
...
gql '{"query":"mutation { createPost(input:{title:\"   \", content:\"c\"}) { id } }"}' > "$LOG/42-error-createPost-blankTitle.txt"
# Bean Validation na borda: título longo demais e update com título só de espaços
LONG=$(python3 -c 'print("x"*201)')
gql "{\"query\":\"mutation { createPost(input:{title:\\\"$LONG\\\", content:\\\"c\\\"}) { id } }\"}" > "$LOG/43-error-createPost-longTitle.txt"
...
# ---- 7. queries ----------------------------------------------------------------------------
gql "{\"query\":\"{ post(id: \\\"$A\\\") { id title content author { name } createdAt updatedAt version tags(first: 5) { edges { cursor node { id name } } pageInfo { hasNextPage } } } }\"}" > "$LOG/50-query-post-A.txt"
